...
const adminLogin = TryCatch(async (req, res, next) => {
    ...
    return res
    .status(200)
    .cookie("jitu-admin-token", token, {...cookieOptions, maxAge: 1000*50*15,})
    .json({
        success: true,
...
const adminLogout = TryCatch(async (req, res, next) => {
    ...
    return res
    .status(200)
    .cookie("jitu-admin-token", "", {...cookieOptions, maxAge: 0,})
    .json({
        success: true,
        message: "Logout Successfully, Bye BOSS, Have a nice day :)",
    });
...
const allMessages = TryCatch(async (req, res) => {
    ...
    const transformMMessages = messages.map(({ content, attachments, _id, sender, createdAt, chat }) => ({
        _id,
        attachments,
        content,
        createdAt,
    ...
    return res.status(200).json({
        success: true,
        messages: transformMMessages,
    });
});

const getDashboardStats = TryCatch(async (req, res) => {

    const [groupsCount,usersCount,messagesCount,TotalsChatCount] = await Promise.all([
